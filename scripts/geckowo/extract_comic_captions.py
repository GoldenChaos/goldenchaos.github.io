...
async def main():
    ...
    comic_files = sorted(comics_dir.glob("*.jpg"))
    
    print(f"Found {len(comic_files)} comics")
    print("Extracting captions with browser rendering...\n")
    
    # Load existing captions if any
    captions_file = Path("data/geckowo/geckowo_comic_captions.json")
    ...
    post_ids = [f.stem for f in comic_files]
    ...
    for i, post_id in enumerate(post_ids, 1):
        ...
        if i % 5 == 0:
            ...
            progress = i
            found = success_count
            total = len([v for v in captions.values() if v is not None])
            print(f"  Progress saved ({progress}/{len(post_ids)}) - Found: {total}\n")
