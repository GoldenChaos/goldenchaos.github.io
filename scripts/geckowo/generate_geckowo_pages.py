"""Generate Geckowo comics and doodles pages from JSON captions"""
import json
from pathlib import Path
from datetime import datetime

# Load captions
comic_captions = json.load(open('data/geckowo/geckowo_comic_captions.json', encoding='utf-8'))
doodle_captions = json.load(open('data/geckowo/geckowo_doodle_captions.json', encoding='utf-8'))

# Sort by ID (chronological)
comics_list = sorted(comic_captions.keys())
doodles_list = sorted(doodle_captions.keys())

def create_page(post_type, post_id, index, total, caption):
    """Create a comic/doodle page HTML"""
    
    # Determine paths
    if post_type == 'comic':
        base_url = '../../'
        nav_back = '../'
        img_path = f'../../images/geckowo_archive/comics/{post_id}.jpg'
        next_index = index + 1
        prev_index = index - 1
        next_id = comics_list[next_index] if next_index < len(comics_list) else None
        prev_id = comics_list[prev_index] if prev_index >= 0 else None
        folder = Path(f'geckowo/comics/{index + 1}')
        title_prefix = "Geckowo Comics"
    else:  # doodle
        base_url = '../../'
        nav_back = '../'
        img_path = f'../../images/geckowo_archive/doodles/{post_id}.jpg'
        next_index = index + 1
        prev_index = index - 1
        next_id = doodles_list[next_index] if next_index < len(doodles_list) else None
        prev_id = doodles_list[prev_index] if prev_index >= 0 else None
        folder = Path(f'geckowo/doodles/{index + 1}')
        title_prefix = "Geckowo Doodles"
    
    # Create folder
    folder.mkdir(parents=True, exist_ok=True)
    
    # Build title
    if caption:
        display_title = f"#{index + 1} - {caption}"
        page_title = f"{title_prefix} - #{index + 1} {caption}"
    else:
        display_title = f"#{index + 1}"
        page_title = f"{title_prefix} - #{index + 1}"
    
    # Navigation links
    if index == 0:
        first_link = '<a disabled>'
        prev_link = '<a disabled>'
    else:
        first_link = f'<a href="../1/">'
        prev_link = f'<a href="../{prev_index}/">'
    
    if index == total - 1:
        next_link = '<a disabled>'
        latest_link = '<a disabled>'
    else:
        next_link = f'<a href="../{next_index + 2}/">'
        latest_link = f'<a href="../">'
    
    view_all_link = f'<a href="../all/">'
    
    # Determine relative paths for stylesheets
    rel_style = '../' * (len(str(index + 1).split('/')) + 1) + 'style.css'
    rel_style = '../../style.css'
    
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-317947-1"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag() {{ dataLayer.push(arguments); }}
        gtag('js', new Date());
        gtag('config', 'UA-317947-1');
    </script>
    <meta charset="utf-8">
    
    <!-- HTML Meta Tags -->
    <title>{page_title}</title>
    <meta name="description" content="{page_title}">
    
    <!-- Facebook Meta Tags -->
    <meta property="og:url" content="https://goldenchaos.net/geckowo/{post_type}s/{index + 1}/">
    <meta property="og:type" content="website">
    <meta property="og:title" content="{page_title}">
    <meta property="og:description" content="Comics and doodles by Claire (Geckowo)!">
    <meta property="og:image" content="https://goldenchaos.net/images/geckowo_archive/{post_type}s/{post_id}.jpg">
    
    <!-- Twitter Meta Tags -->
    <meta name="twitter:card" content="summary_large_image">
    <meta property="twitter:domain" content="goldenchaos.net">
    <meta property="twitter:url" content="https://goldenchaos.net/geckowo/{post_type}s/{index + 1}/">
    <meta name="twitter:title" content="{page_title}">
    <meta name="twitter:description" content="Comics and doodles by Claire (Geckowo)!">
    <meta name="twitter:image" content="https://goldenchaos.net/images/geckowo_archive/{post_type}s/{post_id}.jpg">
    
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="author" content="Claire (Geckowo)">
    <link rel="stylesheet" type="text/css" href="{rel_style}">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css">
    <script src="https://kit.fontawesome.com/ede7ed9b9e.js" crossorigin="anonymous"></script>
    <link rel="icon" href="favicon.ico" type="image/x-icon">
    <link rel="apple-touch-icon" href="apple-touch-icon-precomposed.png">
    <link rel="mask-icon" href="website_icon.svg" color="#4083a0">
    <meta name='viewport' content='initial-scale=1, viewport-fit=cover'>
    
    <script>
        document.addEventListener('DOMContentLoaded', function () {{
            if (window.sessionStorage.getItem('animated') === null) {{
                ['about', 'segmented-control', 'chevron'].forEach(function(id) {{
                    var el = document.getElementById(id);
                    if (el && el.classList) el.classList.add('animate');
                }});
                window.sessionStorage.setItem('animated', 1);
            }}
        }});
    </script>
</head>
<body>
    <div class="page-top white goldenchaos-btt">
        <nav>
            <a id="logo" class="back-logo" href="../">
                <i class="fas fa-chevron-left mobile"></i>
            </a>
            <div class="menu">
                <div>
                    <a class="social" href="https://twitch.tv/jessrappaport" target="_blank"><i class="fab fa-twitch"></i></a>
                    <a class="social" href="https://discord.gg/kp3s6ysCtn" target="_blank" style="display:none;"><i class="fab fa-discord"></i></a>
                    <a class="social" href="https://twitter.com/goldenchaos" target="_blank"><i class="fa-brands fa-x-twitter"></i></a>
                    <a class="social" href="https://instagram.com/goldenchaos" target="_blank"><i class="fab fa-instagram"></i></a>
                    <a class="social" href="https://threads.net/@goldenchaos" target="_blank"><i class="fa-brands fa-threads"></i></a>
                    <a class="social" href="https://bsky.app/profile/goldenchaos.bsky.social" target="_blank"><i class="fa-brands fa-bluesky"></i></a>
                    <a class="social" href="https://www.linkedin.com/in/goldenchaos" target="_blank"><i class="fab fa-linkedin"></i></a>
                    <a class="social" href="mailto:jess@goldenchaos.net" style="display:none;"><i class="fa fa-envelope-square"></i></a>
                </div>
                <div class="desktop">
                    <a href="../../">
                        <i class="fas fa-chevron-left" style="font-size:14px;margin-right:3px;"></i> Back to Home
                    </a>
                </div>
            </div>
        </nav>
    </div>
    <div class="content index page" id="comic-title">
        <h1 id="comics" class="title" style="text-align:center;margin:0;text-transform:none;">{title_prefix}</h1>
    </div>
    {f'<div class="content index page" id="comic-title"><h2 class="title" style="text-align:center;margin:0;text-transform:none;">{display_title}</h2></div>' if caption else ''}
    <div class="content page" style="text-align:center;">
        <img src="{img_path}" alt="{display_title}" title="{display_title}" class="comic-img">
        <div class="segmented-control">
            {first_link}
                <center>
                    <i class="fa-solid fa-fast-backward"></i>
                    First
                </center>
            </a>
            {prev_link}
                <center>
                    <i class="fa-solid fa-backward"></i>
                    Previous
                </center>
            </a>
            {view_all_link}
                <center>
                    <svg viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg" style="width: 26px; height: 26px; display: block; margin: 0 auto 8px;">
                        <rect x="2" y="2" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="11" y="2" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="20" y="2" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="2" y="11" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="11" y="11" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="20" y="11" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="2" y="20" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="11" y="20" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                        <rect x="20" y="20" width="7" height="7" rx="1.5" ry="1.5" fill="currentColor"/>
                    </svg>
                    View All
                </center>
            </a>
            {next_link}
                <center>
                    <i class="fa-solid fa-forward"></i>
                    Next
                </center>
            </a>
            {latest_link}
                <center>
                    <i class="fa-solid fa-fast-forward"></i>
                    Latest
                </center>
            </a>
        </div>
        <div class="segmented-control">
            <a id="copyBtn" onclick="copyComicLink()" style="cursor:pointer;">
                <center>
                    <i class="fa-solid fa-link"></i>
                    Copy Link
                </center>
            </a>
            <a id="copyImgBtn" onclick="copyComicImageLink()" style="cursor:pointer;">
                <center>
                    <i class="fa-solid fa-image"></i>
                    Copy Image Link
                </center>
            </a>
        </div>
    </div>
    <footer>
        <span>&copy; 2004-<script>document.write(new Date().getFullYear())</script> Jess Rappaport</span>
    </footer>
</body>
<script>
function copyComicLink() {{
    navigator.clipboard.writeText(window.location.href);
    const btn = document.getElementById('copyBtn');
    const originalHTML = btn.innerHTML;
    btn.innerHTML = '<center><i class="fa-solid fa-check"></i>Copied!</center>';
    setTimeout(() => {{
        btn.innerHTML = originalHTML;
    }}, 2000);
}}
function copyComicImageLink() {{
    const img = document.querySelector('.comic-img');
    const imagePath = img.src;
    const fullURL = new URL(imagePath, window.location.origin).href;
    navigator.clipboard.writeText(fullURL);
    const btn = document.getElementById('copyImgBtn');
    const originalHTML = btn.innerHTML;
    btn.innerHTML = '<center><i class="fa-solid fa-check"></i>Copied!</center>';
    setTimeout(() => {{
        btn.innerHTML = originalHTML;
    }}, 2000);
}}
</script>
</html>
"""
    
    # Write file
    index_file = folder / 'index.html'
    index_file.write_text(html, encoding='utf-8')
    
    return str(folder)

# Generate all comic pages
print("Generating Geckowo Comics pages...")
for i, post_id in enumerate(comics_list):
    caption = comic_captions[post_id]
    create_page('comic', post_id, i, len(comics_list), caption)
    if (i + 1) % 50 == 0:
        print(f"  Created {i + 1}/{len(comics_list)} comic pages")

print(f"  ✓ All {len(comics_list)} comic pages created!")

# Generate all doodle pages
print("\nGenerating Geckowo Doodles pages...")
for i, post_id in enumerate(doodles_list):
    caption = doodle_captions[post_id]
    create_page('doodle', post_id, i, len(doodles_list), caption)
    if (i + 1) % 10 == 0:
        print(f"  Created {i + 1}/{len(doodles_list)} doodle pages")

print(f"  ✓ All {len(doodles_list)} doodle pages created!")

print("\n✓ Done! Created:")
print(f"  • {len(comics_list)} comic pages at /geckowo/comics/1/ through /geckowo/comics/{len(comics_list)}/")
print(f"  • {len(doodles_list)} doodle pages at /geckowo/doodles/1/ through /geckowo/doodles/{len(doodles_list)}/")

